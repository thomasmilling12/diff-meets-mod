...
.logo{font-size:1.3rem;font-weight:700;color:#5865f2}.nav-links{display:flex;gap:.75rem;margin-left:auto}
...
.tabs{display:flex;gap:0;margin-top:1.5rem;border-bottom:2px solid #30363d}
.tab{padding:.6rem 1.25rem;font-size:.875rem;color:#8b949e;text-decoration:none;border-bottom:2px solid transparent;margin-bottom:-2px}
.tab:hover{color:#e6edf3}.tab.active{color:#5865f2;border-bottom-color:#5865f2}
.section{margin-top:1.5rem}h2{color:#e6edf3;margin-bottom:.25rem}h3{color:#e6edf3;margin-bottom:.5rem;margin-top:1.25rem}
...
.info-box{background:#161b22;border:1px solid #30363d;border-radius:8px;padding:1rem 1.5rem;margin-top:.75rem;font-size:.875rem}
.info-row{display:flex;gap:1rem;padding:.3rem 0;border-bottom:1px solid #21262d}
.info-row:last-child{border-bottom:none}.info-label{color:#8b949e;min-width:160px}.info-value{color:#e6edf3}
...
function header(token: string, guildId?: string): string {
  ...
  <div class="nav-links">
    ...
    ${guildId ? `<a href="/dashboard/guild/${guildId}${tp}">Cases</a>
...
<div class="main">
  ...
  <div class="grid">${guilds.map(g => `<div class="card">
    ...
    <a href="/dashboard/guild/${g.guild_id}${tp}">View Cases →</a>
  </div>`).join("")}</div>
</div>`));
});
...
<div class="main">
  <h2>Guild: <code style="font-size:.95rem">${guildId}</code></h2>
  <h3>Cases (${totalCases} total)</h3>
  <table>
...
router.get("/guild/:guildId/search", (req, res) => {
  ...
  if (query) {
    ...
    const userCases = isId
      ...
      : db.prepare("SELECT * FROM cases WHERE guild_id = ? AND user_tag LIKE ? ORDER BY case_number DESC").all(guildId, `%${query}%`);
    const userWarnings = isId
      ...
      : db.prepare("SELECT * FROM warnings WHERE guild_id = ? AND user_tag LIKE ? ORDER BY created_at DESC").all(guildId, `%${query}%`);
    const userTickets = isId
      ...
      : db.prepare("SELECT * FROM tickets WHERE guild_id = ? AND user_tag LIKE ? ORDER BY created_at DESC").all(guildId, `%${query}%`);

    const cases = userCases as unknown as Array<{ case_number: number; action: string; user_tag: string; user_id: string; moderator_tag: string; reason: string; created_at: number }>;
    const warnings = userWarnings as unknown as Array<{ id: number; user_tag: string; reason: string; moderator_tag: string; created_at: number }>;
    const tickets = userTickets as unknown as Array<{ id: number; user_tag: string; status: string; claimed_by_tag: string | null; created_at: number }>;

    resultsHtml = `
    <h3 style="margin-top:1.5rem">Cases (${cases.length})</h3>
    <table><tr><th>#</th><th>Action</th><th>User</th><th>Moderator</th><th>Reason</th><th>Date</th></tr>
    ${cases.map(c => `<tr><td>#${c.case_number}</td>
      <td><span class="badge" style="background:${ACTION_COLORS[c.action] ?? "#5865f2"}">${c.action}</span></td>
    ...
    <h3 style="margin-top:1.5rem">Warnings (${warnings.length})</h3>
    <table><tr><th>ID</th><th>User</th><th>Reason</th><th>Moderator</th><th>Date</th></tr>
    ${warnings.map(w => `<tr><td>#${w.id}</td><td>${w.user_tag}</td>
    ...
    <h3 style="margin-top:1.5rem">Tickets (${tickets.length})</h3>
    <table><tr><th>#</th><th>User</th><th>Status</th><th>Claimed By</th><th>Opened</th></tr>
    ${tickets.map(t => `<tr><td>#${t.id}</td><td>${t.user_tag}</td>
...
<div class="main">
  <h2>Search User</h2>
  <p class="muted">Enter a Discord user tag (e.g. thomas#0) or user ID (17–19 digit number)</p>
  <form class="search-bar" method="GET" action="/dashboard/guild/${guildId}/search">
    ${token ? `<input type="hidden" name="token" value="${token}">` : ""}
    <input type="text" name="q" value="${query.replace(/"/g, "&quot;")}" placeholder="Username or User ID">
